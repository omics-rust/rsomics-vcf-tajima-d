...
/// windows of --window-size bp, and reports Tajima's D — byte-identical to
/// vcftools --TajimaD.
#[derive(Parser, Debug)]
#[command(name = "rsomics-vcf-tajima-d", version, about, long_about = None)]
pub struct Cli {
...
impl Cli {
    ...
    fn execute(self) -> rsomics_common::Result<()> {
        let rows = compute_tajima_d(&self.vcf, self.window_size)?;
        if self.common.json {
            let env = json!({
